...
const (
	...
	ViewProject         = "SELECT p.id, p.project_name, p.client_name, p.deadline, p.status, p.budget, p.proposal_link, p.assign, p.resource_link, p.user_id FROM projects p INNER JOIN users u ON u.id = p.user_id WHERE u.id = ?;"
	ViewProjectAdmin    = "SELECT * FROM projects"
)
